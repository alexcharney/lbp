...
## MAIN RNAEDITING DATA ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
rna <- readRDS("/sc/arion/work/charna02/symlinks/lbp/liharska2021/final.everything.RDS")
met <- rna$covariates
vob <- readRDS("/sc/arion/projects/psychgen/lbp/data/PEER/LIVPM_20OCT2022_RNA_PRO_RNAEDITING.RDS")$rnaediting$peerVobjects$QC2CALLSET
met[mymet_postmortem==1,IID_ISMMS:=gsub("_", "-", IID_ISMMS)]

## SUBSET RNAEDITING DATA FOR SAMPLES ANALYZED IN EQTL ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
emt <- readRDS("/sc/arion/projects/psychgen/lbp/data/PEER/LIVPM_20OCT2022_RNA_PRO_RNAEDITING.RDS")$rnaediting$peerSampleList$sid
kpMet <- met[SAMPLE_ISMMS %in% emt]
kpVob <- vob[,kpMet$IID_ISMMS]
colnames(kpVob) <- kpMet$SAMPLE_ISMMS

## EQTL RESULTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
qtl <- readRDS("RESULTS_UPDATED24OCT2022.RDS")
eql <- qtl$fullResults$RNAEDITING$nsv11$QC2CALLSET
eql <- eql[!is.na(IDTopVar)]
gLst <- eql$gene[gIx1:gIx2]
    
## EQTL GENOTYPES ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
gty <- fread("RESULTS_UPDATED24OCT2022_RNAEDITING_QC2CALLSET_nsv11_IDTopVar.traw", na=c("NA", ".", "", "-9"))
gty[,`(C)M`:=NULL]
...
## ADD DE SUMSTATS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
add <- as.data.table(read_excel("/sc/arion/projects/psychgen/lbp/scratch/DEsites.xlsx"))
add <- add[,.(gene=Site, livpmDE_Delta=Delta, livpmDE_padj=adj.P.Val)]
...
myOrd <- c("gene", "sampleset", "nLIV", "nPM",
           ...
           "livpmDE_Delta", "livpmDE_padj", "eqtl_ve", "livpm_ve", "sex_ve", "deplbatch_ve", "neu_ve")
